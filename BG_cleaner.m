...
function BG_cleaner(infile, xbound, ybound, zbound, outfile, voxelsize, origin, datatype)
% Inputs:
...
Vm(1:size(Vm,1)-xbound(1,2),:,:) = 0;
Vm(:,1:size(Vm,2)-ybound(1,2),:) = 0;
Vm(:,:,1:zbound(1,1)) = 0;
Vm(size(Vm,1)-xbound(1,1):end,:,:) = 0;
Vm(:,size(Vm,2)-ybound(1,1):end,:) = 0;
Vm(:,:,zbound(1,2):end) = 0;
...
V = make_nii(Vm, voxelsize, vincitomatorigin, datatype);
