...
%observeddifference = (nanmean(sample1) - nanmean(sample2))/(sqrt((std(sample1)^2/size(sample1,1))+(std(sample2)^2/size(sample2,1))));
...
if plotresult
    ...
    ylabel('Count')
    od = plot(observeddifference, 0, '*r', 'DisplayName', sprintf('Observed difference %.3f\nEffect size: %.2f,\np = %f', observeddifference, effectsize, p));
    legend(od);
end
